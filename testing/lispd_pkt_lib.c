...
int pkt_get_mapping_record_length(lispd_mapping_elt *mapping)           //used
{
...
/*    if (mapping->iid >= 0)
        ident_len += sizeof(lispd_pkt_lcaf_t) + sizeof(lispd_pkt_lcaf_iid_t);*/			//modified
...
{
    uint16_t                *afi_ptr;
//    lispd_pkt_lcaf_t        *lcaf_ptr;				//modified
    ...
    lisp_addr_t                             *probed_rloc)
{
    ...
    int                                     ctr = 0;
/*#ifdef LISPMOBMH
    iface_list_elt *elt=NULL;						//need it ??
#endif*/
